  ...
  return (
    ...
    >
      {message.role === 'assistant' && (
        <motion.div 
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#08CB00] to-[#000000] dark:from-[#08CB00] dark:to-[#000000] text-white shadow-lg"
      ...
      <motion.div
        className={`max-w-[80%] rounded-2xl px-6 py-4 shadow-lg backdrop-blur-sm relative overflow-hidden ${
          message.role === 'user'
            ? 'bg-gradient-to-br from-[#08CB00] to-[#000000] dark:from-[#08CB00] dark:to-[#000000] text-[#EEEEEE] dark:text-[#EEEEEE]'
            : 'bg-[#EEEEEE]/95 dark:bg-[#000000]/95 text-[#000000] dark:text-[#EEEEEE] border border-[#08CB00]/30 dark:border-[#08CB00]/30'
        }`}
        whileHover={{ 
          scale: 1.02,
      ...
      >
        ...
        {message.role === 'assistant' && (
          <motion.div
            className="absolute inset-0 bg-gradient-to-r from-[#08CB00]/10 to-[#000000]/10 dark:from-[#08CB00]/10 dark:to-[#EEEEEE]/10"
            initial={{ x: '-100%' }}
            animate={{ x: '100%' }}
      ...
      {message.role === 'user' && (
        <motion.div 
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#08CB00] to-[#000000] dark:from-[#08CB00] dark:to-[#EEEEEE] text-white dark:text-[#000000] shadow-lg"
          whileHover={{ scale: 1.1, rotate: -5 }}
          transition={{ type: "spring", stiffness: 400, damping: 17 }}
        >
          <User className="h-5 w-5 text-[#EEEEEE] dark:text-[#000000]" />
        </motion.div>
      )}
    </motion.div>
  );
